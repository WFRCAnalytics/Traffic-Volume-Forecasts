...
//
      ...
      _createIconNodes: function(box) {
        ...
        if (this.createMoreIcon) {
          //1.openAtStartNode widget in __more btn
          for (i = this.headerIconCount; i < allIconConfigs.length; i++) {
            iconConfig = allIconConfigs[i];
            if (iconConfig.openAtStart) {
              openAtStartNode = iconConfig;//finding openAtStart
            }
          }
          if (openAtStartNode && !this.openAtStartWidget && !this.morePane) {
            this._showMorePane();//create nodes
            this.morePane.openNodebyId(openAtStartNode.id);//open it
          }
        } else {
          //2.without "__more" btn
          ...
          if (openAtStartNode && !this.openAtStartWidget) {
            ...
            this._onIconClick(openAtStartNode, true);
          }
      ...
      _createDropMenuItem: function(sconfig, groupNode) {
        ...
        node.config = sconfig;
        node.config.groupNode = groupNode;//save groupNode to widgetNode's config
        return node;
      },
      ...
      _showMorePane: function() {
        ...
        //#13788
        // desktop: morePane is positioned relative to map
        // mobile: morePane is positioned relative to main page
        var morePaneParentNode = window.appInfo.isRunInMobile ? jimuConfig.mainPageId : jimuConfig.mapId;
        ...
        this.morePane = new PopupTileNodes({
          ...
          items: moreItems,
          popupContainer: morePaneParentNode
        });
